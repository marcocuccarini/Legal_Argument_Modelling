...
class RK4(Approximator):
    def __init__(self, ads, time=0, name="") -> None:
        super().__init__(ads=ads, time=time, name=name)
        self.name = "RK4"

    def perform_iteration(self, delta, epsilon):

        k1 = self.ads.compute_derivative_at(self.ads.arguable_strength)

        y2 = {}
        for a in k1:
            y2[a] = self.ads.arguable_strength[a] + delta / 2 * k1[a]

        k2 = self.ads.compute_derivative_at(y2)

        y3 = {}
        for a in k2:
            y3[a] = self.ads.arguable_strength[a] + delta / 2 * k2[a]

        k3 = self.ads.compute_derivative_at(y3)

        y4 = {}
        for a in k3:
            y4[a] = self.ads.arguable_strength[a] + delta * k3[a]
        ...
        for a in k4:
            ...
            self.ads.arguable_strength[a] += delta * d
